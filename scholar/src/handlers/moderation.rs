...
use crate::moderation::{self, BanType, ContentFlag, ReportReason, ReportStatus, ReportTarget};
use crate::AppState;
...
pub async fn create_report(
    ...
    let target_type = match req.target_type.to_lowercase().as_str() {
        ...
        "user" => {
            let user_id = req.target_id.parse::<i64>().map_err(|_| {
                (
        ...
        "review" => {
            let review_id = req.target_id.parse::<i64>().map_err(|_| {
                (
                    StatusCode::BAD_REQUEST,
                    Json(json!({"error": "Invalid review ID"})),
                )
            })?;
            ReportTarget::Review(review_id)
        }
        "comment" => {
            let comment_id = req.target_id.parse::<i64>().map_err(|_| {
                (
                    StatusCode::BAD_REQUEST,
    ...
    let reason = match req.reason.to_lowercase().as_str() {
        ...
        _ => {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "Invalid reason"})),
            ))
        }
    };
...
pub async fn get_pending_reports(
    ...
    let reports = moderation::get_pending_reports(&*state.db.conn(), 100).map_err(|e| {
        tracing::error!("Failed to get reports: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to get reports"})),
        )
    })?;
...
pub async fn ban_user(
    ...
    let ban_type = match req.ban_type.to_lowercase().as_str() {
        ...
        _ => {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "Invalid ban type"})),
            ))
        }
    };
    ...
    .map_err(|e| {
        ...
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to ban user"})),
        )
    })?;
...
pub async fn unban_user(
    ...
    moderation::unban_user(&*state.db.conn(), user_id).map_err(|e| {
        tracing::error!("Failed to unban user: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to unban user"})),
        )
    })?;
...
pub async fn get_active_bans(
    ...
    let bans = moderation::get_active_bans(&*state.db.conn()).map_err(|e| {
        tracing::error!("Failed to get bans: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to get bans"})),
        )
    })?;
...
pub async fn flag_content(
    ...
    .map_err(|e| {
        ...
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to flag content"})),
        )
    })?;
...
pub async fn unflag_content(
    ...
    let flag_type = req
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "flag_type required"})),
            )
        })?;

    moderation::remove_flag(&*state.db.conn(), &file_uuid, flag_type).map_err(|e| {
        tracing::error!("Failed to remove flag: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to remove flag"})),
        )
    })?;
...
pub async fn get_flagged_content(
    ...
    let flags = moderation::get_flagged_content(&*state.db.conn()).map_err(|e| {
        tracing::error!("Failed to get flags: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to get flagged content"})),
        )
    })?;
...
pub async fn get_moderation_log(
    ...
    let log = moderation::get_moderation_log(&*state.db.conn(), limit).map_err(|e| {
        tracing::error!("Failed to get moderation log: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to get moderation log"})),
        )
    })?;
...
pub async fn check_user_ban(
    ...
    let is_banned = moderation::is_user_banned(&*state.db.conn(), user.id).map_err(|e| {
        tracing::error!("Failed to check ban status: {}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "Failed to check ban status"})),
        )
    })?;
