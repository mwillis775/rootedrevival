...
pub async fn login(
    ...
    match state.db.authenticate_user(&req.username, &req.password) {
        Ok(Some(user)) => match state.db.create_session(user.id, None, None) {
            Ok(session) => (
                StatusCode::OK,
                Json(json!({
                    "success": true,
...
pub async fn me(State(state): State<Arc<AppState>>, headers: HeaderMap) -> impl IntoResponse {
    ...
    match state.db.validate_session(&token) {
        Ok(Some((_, user))) => (StatusCode::OK, Json(json!({ "user": user.public_view() }))),
        _ => (
            StatusCode::UNAUTHORIZED,
...
pub async fn get_profile(
    ...
    match state.db.get_user_by_username(&username) {
        Ok(Some(user)) => {
            ...
            let files = state
                .db
                .get_files_by_user(user.id, 20, 0)
                .unwrap_or_default();
...
pub async fn update_profile(
    ...
    match state.db.validate_session(&token) {
        Ok(Some((_, user))) => {
            ...
            match state
                .db
                .update_user(user.id, display_name, bio, affiliation)
            {
...
pub async fn forgot_password(
    ...
    let base_url =
        std::env::var("BASE_URL").unwrap_or_else(|_| "http://localhost:8889".to_string());
    if let Err(e) = state
        .email
        .send_password_reset(
...
pub async fn resend_verification(
    ...
    if let Err(e) = state
        .email
        .send_verification(&email, &user.username, &verify_token, &base_url)
        .await
    {
