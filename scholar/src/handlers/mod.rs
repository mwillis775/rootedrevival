...
pub fn api_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/moderation/flags/:file_uuid",
            delete(moderation::unflag_content),
        )
...
async fn get_work_type_info(
    ...
    match WorkType::from_str(&work_type) {
        ...
        None => {
            let valid_types: Vec<&str> = WorkType::all().iter().map(|wt| wt.as_str()).collect();
