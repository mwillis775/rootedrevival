...
impl Database {
    ...
    pub fn create_session(
        ...
        conn.execute(
            ...
            VALUES (?1, ?2, ?3, ?4, ?5)
            "#,
            params![user_id, token, expires_at.to_rfc3339(), ip, user_agent,],
        )?;
    ...
    pub fn validate_session(&self, token: &str) -> anyhow::Result<Option<(Session, User)>> {
        ...
        match result {
            Ok((id, user_id, expires_str)) => {
                let expires_at = DateTime::parse_from_rfc3339(&expires_str)?.with_timezone(&Utc);
