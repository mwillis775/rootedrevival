...
use crate::models::{
    Citation, CitationType, ContentContext, DomainReputation, File, FileVersion,
    ModerationReasoning, NewFile, ResultType, VersionStatus, VisibilityStatus, WorkType,
};
...
impl Database {
    ...
    pub fn get_files_by_tag(
        ...
        let files: Vec<File> = stmt
            .query_map(params![tag.to_lowercase(), limit, offset], |row| {
                File::from_row(row)
            })?
            .filter_map(|r| r.ok())
            .collect();
    ...
    pub fn get_domain_reputation(
        ...
        user_id: i64,
        domain: WorkType,
    ) -> anyhow::Result<Option<DomainReputation>> {
        let conn = self.conn();
    ...
    pub fn get_all_domain_reputations(
        &self,
        user_id: i64,
    ) -> anyhow::Result<Vec<DomainReputation>> {
        let conn = self.conn();
            ...
            .query_map(params![file_uuid], |row| {
                ...
                Ok(FileVersion {
                    ...
                    status: VersionStatus::from_str(&status_str)
                        .unwrap_or(VersionStatus::Published),
    ...
    pub fn get_citations_from(&self, file_uuid: &str) -> anyhow::Result<Vec<Citation>> {
        ...
        let mut stmt =
            conn.prepare("SELECT * FROM citations WHERE citing_file_uuid = ?1 ORDER BY id")?;
    ...
    pub fn get_content_contexts(&self, file_uuid: &str) -> anyhow::Result<Vec<ContentContext>> {
        ...
        let mut stmt = conn.prepare("SELECT * FROM content_contexts WHERE file_uuid = ?1")?;
    ...
    pub fn add_moderation_reasoning(
        ...
        let appeal_deadline =
            appeal_deadline_days.map(|days| format!("datetime('now', '+{} days')", days));
    ...
    pub fn get_moderation_reasoning(
        &self,
        moderation_log_id: i64,
    ) -> anyhow::Result<Option<ModerationReasoning>> {
        let conn = self.conn();
